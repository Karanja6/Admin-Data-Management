  ...
  const handleNotificationChange = (type: 'email' | 'push' | 'sms', value: boolean) => {
    setEditedUser({
      ...
      preferences: {
       ...
       notifications: {
  email: editedUser.preferences?.notifications?.email ?? false,
  push: editedUser.preferences?.notifications?.push ?? false,
  sms: editedUser.preferences?.notifications?.sms ?? false,
  [type]: value // overrides the target key
}
      }
    });
  };
...
type UserRole = 'client' | 'admin' | 'guest';
          {/* Edit Actions */}
          {isEditing && (
            <Card>
